...
/* ================= TYPES ================= */
export interface SigningEditorProps {
  file: File;
...
/* ================= COMPONENT ================= */
export const SigningEditor: React.FC<SigningEditorProps> = ({
  ...
  return (
    <div style={{ display: "flex", gap: "24px", alignItems: "flex-start" }}>
      {/* LEFT COLUMN - Document + Button */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "20px" }}>
        {/* Toolbar */}
        <div style={{ display: "flex", gap: "8px", padding: "12px 16px", background: t.toolbarBg, backdropFilter: "blur(16px)", borderRadius: "14px", border: `1px solid ${t.toolbarBorder}`, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
          {tools.map((tool) => (
            <button
              key={tool.id}
              ...
              style={{
                display: "inline-flex", alignItems: "center", gap: "6px", padding: "7px 14px", borderRadius: "9px", border: "none",
                background: selectedTool === tool.id ? t.toolActiveBg : hoveredTool === tool.id ? t.toolHoverBg : "transparent",
                color: selectedTool === tool.id ? t.toolActiveColor : "#64748b", fontSize: "13px", fontWeight: 600, cursor: "pointer", transition: "all 0.2s",
              }}
            >
            ...
            {signatureBoxes.map((box) => {
              ...
              return (
                <div
                  ...
                  className="group"
                  style={{
                    position: "absolute", left: box.xRatio * pdfSize.width, top: box.yRatio * pdfSize.height, width: box.widthRatio * pdfSize.width, height: box.heightRatio * pdfSize.height,
                    border: `2px dashed ${isSelfSign ? '#f59e0b' : `hsl(var(--${box.color}))`}`, borderRadius: "10px", background: isSelfSign ? 'rgba(245, 158, 11, 0.05)' : 'transparent',
                    cursor: selectedTool === "move" && mode !== "sign" ? "move" : "default",
                  }}
                >
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100" style={{ background: isSelfSign ? '#f59e0b' : `hsl(var(--${box.color}))`, color: "#fff", whiteSpace: "nowrap", fontWeight: 600 }}>
                    {isSelfSign ? "Self Signature" : signer?.name}
                  </div>
                  {mode === "sign" && !box.signed && (
                    <button onClick={() => handleSignDocument(box)} disabled={processing} style={{ position: "absolute", bottom: "4px", right: "4px", padding: "4px 12px", borderRadius: "6px", border: "none", background: isSelfSign ? 'linear-gradient(135deg, #f59e0b, #d97706)' : t.gradient, color: "white", fontSize: "11px", fontWeight: 600, cursor: processing ? "not-allowed" : "pointer", boxShadow: t.btnShadow }}>
                      {isSelfSign ? "Self Sign" : "Sign"}
                    </button>
                  )}
                  {box.signed && (
                    <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", background: "rgba(22, 163, 74, 0.9)", color: "white", padding: "4px 12px", borderRadius: "6px", fontSize: "11px", fontWeight: 700, pointerEvents: "none" }}>✓ Signed</div>
                  )}
                </div>
              );
            })}
      ...
      <div style={{ width: "300px", flexShrink: 0 }}>
        <div style={{ background: t.sidebarBg, backdropFilter: "blur(16px)", borderRadius: "18px", border: `1px solid ${t.sidebarBorder}`, boxShadow: "0 4px 20px rgba(0,0,0,0.06)", padding: "24px", position: "sticky", top: "24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {signingType === "self" ? (
              signatureBoxes.map((box) => (
                <div key={box.id} style={{ padding: "14px 16px", borderRadius: "12px", border: `1px solid ${t.selfSignBorder}`, background: box.signed ? t.signedBg : t.selfSignBg }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
                    <div style={{ width: "30px", height: "30px", borderRadius: "8px", background: "rgba(245, 158, 11, 0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}><User size={14} color="#f59e0b" /></div>
                    <span style={{ fontSize: "14px", fontWeight: 600, color: "#0f172a" }}>You (Self)</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <Clock size={12} color={box.signed ? t.signedColor : t.pendingColor} />
                    <span style={{ fontSize: "12px", fontWeight: 500, color: box.signed ? t.signedColor : t.pendingColor }}>{box.signed ? "Signed ✓" : "Pending"}</span>
                  </div>
                </div>
              ))
            ) : (
              signers.map((signer) => {
                ...
                return (
                  <div key={signer.id} style={{ padding: "14px 16px", borderRadius: "12px", border: `1px solid hsl(var(--${signer.color}))`, background: box?.signed ? t.signedBg : t.signerCardBg }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
                      <div style={{ width: "30px", height: "30px", borderRadius: "8px", background: `hsl(var(--${signer.color}) / 0.15)`, display: "flex", alignItems: "center", justifyContent: "center" }}><User size={14} color={`hsl(var(--${signer.color}))`} /></div>
